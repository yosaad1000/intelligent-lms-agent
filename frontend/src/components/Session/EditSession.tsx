...
import { SessionUpdate, SessionFormData, ApiError, SessionResponse } from '../../types';
import { useSessions } from '../../hooks/useSessions';
import { useToast } from '../../contexts/ToastContext';
import { useSessionFormValidation } from '../../hooks/useSessionFormValidation';
import { getCurrentLocalDate, getCurrentLocalTime, createLocalDateTime, formatDateForInput, formatTimeForInput } from '../../utils/dateTimeUtils';
import { 
  XMarkIcon,
  CalendarIcon,
  ClockIcon,
...
const EditSession: React.FC<EditSessionProps> = ({ 
  ...
  const { updateSession, loading } = useSessions(session.subject_id);
  ...
  // Initialize form data when session or modal opens
  useEffect(() => {
    if (isOpen && session) {
      ...
      setFormData({
        ...
        session_date: sessionDate ? formatDateForInput(sessionDate) : getCurrentLocalDate(),
        session_time: sessionDate ? formatTimeForInput(sessionDate) : getCurrentLocalTime(),
      });
      
      // Show advanced section if description exists
      setShowAdvanced(!!session.description);
      resetValidation();
    }
  }, [isOpen, session, resetValidation]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedSession = await updateSession(session.session_id, updateData);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md max-h-[95vh] sm:max-h-[90vh] overflow-y-auto transition-colors">
          ...
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
            <div className="flex-1 min-w-0">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-gray-100">Edit Session</h2>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 hidden sm:block">Update session details</p>
            </div>
            <button
              onClick={handleClose}
              disabled={loading}
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-5">
            ...
            <div>
              <label htmlFor="edit-session-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Session Name
              </label>
              <input
                ...
                aria-describedby={hasError('name') && isFieldTouched('name') ? 'edit-session-name-error' : undefined}
              />
              {hasError('name') && isFieldTouched('name') && (
                <p id="edit-session-name-error" className="text-sm text-red-600 dark:text-red-400 mt-1">
                  {getFieldError('name')}
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <label htmlFor="edit-session-date" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                ...
                <input
                  ...
                  aria-describedby={hasError('session_date') && isFieldTouched('session_date') ? 'edit-session-date-error' : undefined}
                />
                {hasError('session_date') && isFieldTouched('session_date') && (
                  <p id="edit-session-date-error" className="text-sm text-red-600 dark:text-red-400 mt-1">
              ...
              <div>
                <label htmlFor="edit-session-time" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                ...
                <input
                  ...
                  aria-describedby={hasError('session_time') && isFieldTouched('session_time') ? 'edit-session-time-error' : undefined}
                />
                {hasError('session_time') && isFieldTouched('session_time') && (
                  <p id="edit-session-time-error" className="text-sm text-red-600 dark:text-red-400 mt-1">
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
              ...
              {showAdvanced && (
                <div className="mt-4 space-y-4 animate-in slide-in-from-top-2 duration-200">
                  <div>
                    <label htmlFor="edit-session-description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      Description
                    </label>
                    <textarea
                      ...
                      aria-describedby={hasError('description') && isFieldTouched('description') ? 'edit-session-description-error' : undefined}
                    />
                    {hasError('description') && isFieldTouched('description') && (
                      <p id="edit-session-description-error" className="text-sm text-red-600 dark:text-red-400 mt-1">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700 sm:border-t-0">
              ...
              >
                ...
                {validationState.isValidating ? 'Validating...' : loading ? 'Updating...' : 'Update Session'}
              </button>
            </div>
          </form>
        </div>
